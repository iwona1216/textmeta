...
var PDFTextExtractor = function (options) {
  ...
  if (typeof this.options.allWhitespaceRegexp === "string") {
    this.options.allWhitespaceRegexp = new RegExp(this.options.allWhitespaceRegexp);
  }
}
...
PDFTextExtractor.prototype = {
  ...
  isAllWhitespace: function (str) {
    return this.options.allWhitespaceRegexp.test(str);
  },
          ...
          return document.getPage(pageNumber).then((page) => {
            ...
            return page.getTextContent().then((content) => {
              ...
              content.items.map((item) => {
                ...
                if (!item.isAllWhitespace) {
                  pageText += item.str;
                }
                lastItem = item;
                if (content.items.length == index + 1)//if last item on the page, add line break
                  pageText += "\n";
              });
